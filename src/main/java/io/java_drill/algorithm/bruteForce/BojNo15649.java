...
/**
 * 재귀 함수를 이용한 완전 탐색
 * 1과 N 사이의 자연수를 중복이 불가능하게 M개를 출력(1<=M<=N<=8)
 * O(8!) = 약 4만 -> 완전탐색 1초 가능
 */
public class BojNo15649 {
    ...
    static void rec_func(int k) {
        if(k == M + 1) {    // 탐색이 종료 모든 조건을 찾음
            ...
            for(int i=1; i<=N; i++){
                if(used[i] == 1) continue;  // 이미 사용되었다면 중복이므로 불가능

                selected[k] = i;    used[i] = 1;    // 사용을 표시
                rec_func(k+1);
                selected[k] = 0;    used[i] = 0;    // 재귀 후 다시 돌려놓기
            }
        }
    }
